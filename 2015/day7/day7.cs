...
int Compute(string value)
{
    ...
    if (value.Contains("NOT"))
        return ~Compute(value.Split(' ')[1]) & 0xffff;
    ...
    try
    {
        ...
        if (valueComponents[1] == "AND")
            return Compute(valueComponents[0]) & Compute(valueComponents[2]);
        else if (valueComponents[1] == "OR")
            return Compute(valueComponents[0]) | Compute(valueComponents[2]);
        else if (valueComponents[1] == "LSHIFT")
            return Compute(valueComponents[0]) << Compute(valueComponents[2]);
        else if (valueComponents[1] == "RSHIFT")
            return Compute(valueComponents[0]) >> Compute(valueComponents[2]);
    }
    catch
    {
